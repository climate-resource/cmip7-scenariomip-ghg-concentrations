"""
Create one-box model based annual-mean file
"""
...
@task(
    task_run_name="create-one-box-annual-mean-file_{ghg}",
    persist_result=True,
...
def create_one_box_annual_mean_file(  # noqa: PLR0913
    ...
    """
    Create an annual mean file based on one-box projections
    ...
    emissions_complete_dir
        Directory in which complete emissions scenarios are saved
    ...
    call_maybe_in_subprocess(
        run_notebook,
        maybe_pool=pool,
        notebook=raw_notebooks_root_dir / "0030_create-projection-one-box-model.py",
        # verbose=2,
        # progress=True,
        parameters={
            "ghg": ghg,
