...
from cmip7_scenariomip_ghg_generation.mean_preserving_interpolation.grouping import (
    get_group_averages,
    get_group_indexes,
    get_group_integrals,
)
...
class RymesMeyersInterpolator:
    ...
    def __call__(
        ...
        y_starting_values = cast(pint.UnitRegistry.Quantity, np.interp(x_out_mids, x_bounds_in, y_at_boundaries))
    ...
    def lower_bound_adjustment(  # noqa: PLR0913
        ...
        current_vals[np.where(np.isclose(current_vals.m, min_val_same_u.m, atol=self.atol, rtol=self.rtol))] = (
            min_val_same_u
        )
